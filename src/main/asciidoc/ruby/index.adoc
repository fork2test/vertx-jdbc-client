...
The client API is represented with the interface `link:../../yardoc/VertxJdbc/JDBCClient.html[JDBCClient]`.
...
----
compile 'io.vertx:vertx-jdbc-client:3.4.2.1-SNAPSHOT'
----
...
client = VertxJdbc::JDBCClient.create_shared(vertx, config)


----

The first call to `link:../../yardoc/VertxJdbc/JDBCClient.html#create_shared-class_method[JDBCClient.createShared]`
...
client = VertxJdbc::JDBCClient.create_shared(vertx, config, "MyDataSource")
...
In that case you can use `link:../../yardoc/VertxJdbc/JDBCClient.html#create_non_shared-class_method[JDBCClient.createNonShared]`.

[source,java]
----
require 'vertx-jdbc/jdbc_client'

client = VertxJdbc::JDBCClient.create_non_shared(vertx, config)


----

This is equivalent to calling `link:../../yardoc/VertxJdbc/JDBCClient.html#create_shared-class_method[JDBCClient.createShared]`
...
client = VertxJdbc::JDBCClient.create(vertx, dataSource)
...
Once you've created a client you use `link:../../yardoc/VertxJdbc/JDBCClient.html#get_connection-instance_method[getConnection]` to get
a connection.
...
The connection is an instance of `link:../../yardoc/VertxSql/SQLConnection.html[SQLConnection]` which is a common interface not only used by
